...
export default async function DayPage({ params }: Props) {
  ...
  if (isNaN(wk) || isNaN(dy)) notFound();

  const result = await getPurchasedDayTemplate(
    session.user.id,
    "workouts",
    slug,
    wk,
        ...
        <div className="max-w-3xl mx-auto flex flex-col gap-8 sm:gap-10">
          ...
          {sections.map((section) => {
            ...
            return (
              <div
                key={section.type}
                className="flex flex-col rounded-2xl border border-zinc-200 overflow-hidden shadow-sm"
              >
                {/* Section header */}
                <div className="bg-zinc-950 px-5 sm:px-7 py-5 sm:py-6 flex items-center gap-4 sm:gap-5">
                  <div
                    className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl ${meta.bg} flex items-center justify-center shrink-0`}
                  >
                  ...
                  <span className="text-[10px] sm:text-xs font-bold text-white/90 uppercase tracking-widest [font-family:var(--font-barlow)] shrink-0">
                    ...
                    {section.exercises.length === 1 ? "exercise" : "exercises"}
                  </span>
                </div>

                {/* Column headers — desktop only */}
                <div className="hidden sm:grid grid-cols-[1fr_auto] items-center px-7 py-3 bg-zinc-50 border-b border-zinc-100">
                  <span className="text-xs font-black uppercase tracking-widest text-zinc-400 [font-family:var(--font-barlow)]">
                    Exercise
                  </span>
                ...
                <div className="divide-y divide-zinc-100 bg-white">
                  {section.exercises.map((ex, ei) => (
                    <div
                      key={ei}
                      className="px-5 sm:px-7 py-5 sm:py-5 flex flex-col sm:grid sm:grid-cols-[1fr_auto] sm:items-center gap-3 sm:gap-4"
                    >
                      {/* Name + notes */}
                      ...
                      <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 sm:gap-4">
                        ...
                        <span className="inline-flex items-center sm:w-16 sm:justify-center px-3 sm:px-0 py-1.5 sm:py-0 rounded-full sm:rounded-none bg-zinc-950 sm:bg-transparent text-white sm:text-zinc-950 text-xs sm:text-sm font-black uppercase tracking-wide [font-family:var(--font-barlow)]">
                          <span className="sm:hidden mr-1 text-white/50">
                            ×
                          </span>
                          {ex.sets}
                          <span className="sm:hidden ml-0.5 text-white/50 font-semibold text-[10px]">
                            sets
                          </span>
                        </span>
