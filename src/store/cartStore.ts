  ...
  persist(
    (set, get) => ({
      ...
      addItem: (item) => {
        ...
        set({ items: newItems, isOpen: true, ...computeTotals(newItems, appliedCoupon), totalItems: newItems.length })
      },
      removeItem: (slug) => {
        const { items, appliedCoupon } = get()
        const newItems = items.filter((i) => i.slug !== slug)
        set({ items: newItems, ...computeTotals(newItems, appliedCoupon), totalItems: newItems.length })
      },
      ...
      applyCoupon: () => {
        ...
        set({ appliedCoupon, couponError: "", ...computeTotals(items, appliedCoupon) })
      },
      removeCoupon: () => {
        const { items } = get()
        set({ appliedCoupon: null, couponInput: "", couponError: "", ...computeTotals(items, null) })
      },
      setEmail: (v) => set({ email: v }),
    }),
    {
  name: "sg-fit-cart",
  partialize: (state) => ({ items: state.items, appliedCoupon: state.appliedCoupon, email: state.email }),
